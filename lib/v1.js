...
function hydrateString(local, string) {
    var splits = R.flatten(splitStringByRef(string)
    ...
    return db.getMultiple(refs.map(R.prop("key")))
        ...
        .map(obj => {
            if (R.isNil(obj.value)) throw new restify.ResourceNotFoundError([obj.key].concat(obj.props).reverse().join(' of ') + ' not available');
                ...
                var value = R.is(String, obj.value) ? obj.value : JSON.stringify(obj.value);
                local[obj.key] = value;
                return value;
            }
        })
...
var checkLocalStorage = R.curry((local, i, obj) => {
    if (i > 25) {
