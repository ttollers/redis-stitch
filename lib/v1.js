...
function hydrateString(local, string){
    var m, resource, init, tail;
    if (m = string.match(/\${(.*?)}/)) {
        init = string.slice(0, m.index);
        resource = m[1];
        tail = string.slice(m.index + m[0].length);
        return hydrateKey(local, resource).flatMap(
...
function hydrateKey(local, resource){
    var xs = resource.split(',');
    var key = R.head(xs), props = R.tail(xs);
    var stream;
    if (local[key] === null) {
    ...
    return stream
        .map(R.path(props)).map(_ => {
            if (_ == null) throw new restify.ResourceNotFoundError( [key].concat(props).reverse().join(' of ') + ' not available');
            else return _;
        });
}
